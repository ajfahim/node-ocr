...
const API_URL = 'https://node-ocr-kuew.onrender.com/api/ocr/base64';
...
async function runLoadTest() {
  console.log(`Starting load test on deployed Node.js API with ${CONCURRENT_REQUESTS} concurrent requests`);
  ...
  const avgResponseTime = responseData.timing.length > 0 
    ? responseData.timing.reduce((a, b) => a + b, 0) / responseData.timing.length 
    : 0;
  
  responseData.timing.sort((a, b) => a - b);
  const medianResponseTime = responseData.timing.length > 0
    ? responseData.timing[Math.floor(responseData.timing.length / 2)]
    : 0;
  
  const p95ResponseTime = responseData.timing.length > 0
  ...
  fs.writeFileSync(
    path.join(__dirname, 'node-deployed-test-results.json'),
    JSON.stringify({
      config: {
  ...
  console.log('\nResults saved to test/node-deployed-test-results.json');
}
